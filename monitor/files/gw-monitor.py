...
class monitorDb:
    ...
    def set_gw_monitor_keepalive(self):
        if (self._rh_status):
            self._rh_status.pexpire(self._instance, 31 * 1000)

    def get_gw_monitor_keepalive(self):
        val = None
        if (self._rh_status):
            val = self._rh_status.get(self._instance)
        return val
...
class gwOperations():
    ...
    def check_charon(self, oper):
        _gw = str(os.environ['MGW_IP_ADDRESS'])
...
def start_monitoring(gw_to_monitor):
    try:
        ...
        sys.exit(-1)
    _proxy = str(os.environ['MGW_PROXY_ADDRESS'])
...
if __name__ == "__main__":
    ...
    fh = logging.FileHandler('/var/log/gw-monitor.log')
